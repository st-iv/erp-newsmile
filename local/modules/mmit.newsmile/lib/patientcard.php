...
class PatientCardTable extends Entity\DataManager
{
    ...
    {
        $result = parent::add($data); // TODO: Change the autogenerated stub
        if ($result->isSuccess()) {
            $data['ID'] = $result->getId();
            self::indexSearch($data);
        }
        return $result;
    }
    ...
    {
        $result = parent::update($primary, $data); // TODO: Change the autogenerated stub
        if ($result->isSuccess()) {
            $data['ID'] = $primary;
            self::indexSearch($data);
        }
    ...
    private static function indexSearch($data)
    {
        ...
        \CSearch::Index(
            ...
            [
                ...
                "PARAM2" => '',
                "URL" => '',
                "BODY" => $data['LAST_NAME'] . ' ' . $data['NAME'] . ' ' . $data['SECOND_NAME'] . ' ' . $data['PERSONAL_PHONE'] . ' ' . $data['PERSONAL_MOBILE'],
//                    "TAGS" => 'Калинина А.И'
            ],
            true
        );
